...
function registerGuildSchedule(context: AppContext, guildId: string, cronExpression: string): void {
    const task = cron.schedule(cronExpression, async () => {
        try {
            ...
            if (result.skipped) {
                logger.info({ guildId, skippedReason: result.skippedReason }, 'Raid reminder cycle skipped.');
                return;
            }

            logger.info(
                { guildId, candidatesEvaluated: result.candidatesEvaluated, remindersSent: result.remindersSent, remindersSkippedAsDuplicate: result.remindersSkippedAsDuplicate, deliveryFailures: result.deliveryFailures },
                'Raid reminder cycle complete.',
            );
        } catch (error) {
            logger.error({ err: error, guildId }, 'Raid reminder cycle execution failed.');
        }
    });

    guildSchedules.set(guildId, task);
    logger.info({ guildId, cronExpression }, 'Raid reminder schedule registered for guild.');
}
...
export async function refreshGuildRaidReminderSchedule(context: AppContext, guildId: string): Promise<void> {
    ...
    if (!settings || !settings.raidScheduleCron || !settings.raidAttendanceReminderThreshold) {
        logger.info({ guildId }, 'refreshGuildRaidReminderSchedule: no valid schedule configured, skipping registration.');
        return;
    }

    if (!cron.validate(settings.raidScheduleCron)) {
        logger.warn({ guildId, raidScheduleCron: settings.raidScheduleCron }, 'refreshGuildRaidReminderSchedule: invalid cron expression, skipping registration.');
        return;
    }
...
export async function startRaidReminderScheduler(context: AppContext): Promise<void> {
    ...
    for (let attempt = 1; attempt <= SCHEDULER_BOOTSTRAP_MAX_ATTEMPTS; attempt += 1) {
        try {
            ...
            logger.warn(
                { err: error, attempt, maxAttempts: SCHEDULER_BOOTSTRAP_MAX_ATTEMPTS, retryDelayMs: SCHEDULER_BOOTSTRAP_RETRY_DELAY_MS },
    ...
    for (const setting of allSettings) {
        if (!setting.raidScheduleCron || !cron.validate(setting.raidScheduleCron)) {
            logger.warn({ guildId: setting.guildId, raidScheduleCron: setting.raidScheduleCron }, 'startRaidReminderScheduler: invalid or missing cron for guild, skipping.');
            continue;
        }
    ...
    logger.info({ registeredGuilds: registered }, 'Raid reminder scheduler initialized.');
}
