...
export function buildTrialListEmbeds(
	items: TrialListItem[],
	activeOnly: boolean,
	logoUrl?: string,
): EmbedBuilder[] {
	...
	return chunks.map((chunk, chunkIndex) => {
		const embed = new EmbedBuilder()
			...
			.setTitle(activeOnly ? "Active Trials" : "All Trials")
			.setFooter({
				text: `Page ${chunkIndex + 1}/${chunks.length} • ${items.length} total`,
			})
			.setTimestamp(new Date());
...
export function buildFeedbackSummaryEmbed(
	...
	if (result.outcome === "no_feedback") {
		return applyGatekeeperLogo(
			new EmbedBuilder()
				...
				.setTitle("Trial Feedback Summary")
				.setDescription(
					`An active trial exists for **${displayName}**, but no feedback has been submitted yet.`,
				)
				.addFields({
	...
	const comments =
		result.summary.recentComments.length === 0
			...
			.addFields(
				{
					...
					value: String(result.summary.feedbackCount),
					inline: true,
				},
				{
					name: "Late Marks",
					value: String(result.summary.lateCount),
					inline: true,
				},
				{
					name: "Performance",
					value: `${result.summary.averages.performance}/5`,
					inline: true,
				},
				{
					name: "Attitude",
					value: `${result.summary.averages.attitude}/5`,
					inline: true,
				},
				{
					name: "Focus",
					value: `${result.summary.averages.focus}/5`,
					inline: true,
				},
				{
					name: "Expected Completion (Projected)",
...
export function buildRoleDebugEmbed(
	input: RoleDebugEmbedInput,
	logoUrl?: string,
): EmbedBuilder {
		...
		new EmbedBuilder()
			...
			.addFields(
				{ name: "Guild ID", value: input.guildId, inline: false },
				{
					...
					value: String(input.botHighestRolePosition),
					inline: true,
				},
				{
					name: "Managed Roles In Guild",
					value: String(input.managedRoleCount),
					inline: true,
				},
			)
			.setTimestamp(new Date()),
...
export function buildRaidAttendanceReminderEmbed(
	...
	return applyGatekeeperLogo(
		new EmbedBuilder()
			...
			.setDescription(
				`<@${input.userId}> has reached the raid attendance threshold.`,
			)
			.addFields(
				{ name: "Member", value: input.displayName, inline: true },
				{
					name: "Raid Nights Attended",
					value: String(input.raidNightsAttended),
					inline: true,
				},
	...
	return applyGatekeeperLogo(
		new EmbedBuilder()
			...
			.addFields(
				{
					name: "Member",
					value: `${input.memberDisplayName} (<@${input.memberId}>)`,
					inline: false,
				},
				{
					name: "Officer",
					value: `${input.officerDisplayName} (<@${input.officerId}>)`,
					inline: false,
				},
				{
					name: "Started",
					value: formatDiscordTimestamp(input.startedAt),
					inline: true,
				},
				{
					name: "Expected Completion (Projected)",
				...
				},
				{
					name: "Reminder",
					value:
						"Officers: leave `/feedback` after each attended raid to keep this trial on track.",
					inline: false,
				},
			)
			.setTimestamp(new Date()),
...
export function buildTrialResolvedEmbed(
	...
	return applyGatekeeperLogo(
		new EmbedBuilder()
			...
			.setDescription(
				`<@${input.memberId}> has ${isPassed ? "passed" : "failed"} their trial.`,
			)
			.addFields(
				{
					name: "Member",
...
export function buildTrialVotePollEmbed(
	input: TrialVotePollEmbedInput,
	logoUrl?: string,
): EmbedBuilder {
	return applyGatekeeperLogo(
		new EmbedBuilder()
			...
			.setDescription(
				`Vote for **${input.targetDisplayName}** (<@${input.targetId}>).`,
			)
			.addFields(
